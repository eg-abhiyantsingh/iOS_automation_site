# 108 — asset_engineer expansion: 16 → 105 test cases (deep coverage)
...
| AssetEngineerInputs_Test | 20 | int/double input-filter matrices (kVA, % Impedance, Qty per Phase, Length); 4 nondeterministic rows removed across 3-run triangulation (decimal-pad dot drop; trailing-rejected-char + trailing-zero display variance) |
...
| ClassMatrix (16) | ✅ 16/16 confirmed (incl. ATS→CB transition TC_ENG_051) |
| Inputs (20) | 20/20 deterministic set | 4 env-nondeterministic rows removed after 3-run triangulation |
