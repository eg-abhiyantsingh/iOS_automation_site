...
struct CreateSiteView: View {
    ...
    @StateObject private var viewModel: CreateSiteViewModel
    ...
    var onSiteCreated: ((UUID, String) -> Void)? = nil

    @MainActor
    init(onSiteCreated: ((UUID, String) -> Void)? = nil) {
        let repo = SiteRepositoryImpl()
        _viewModel = StateObject(
            wrappedValue: CreateSiteViewModel(
                createSiteUseCase: CreateSiteUseCase(repository: repo),
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(AppStrings.Site.createNewSite)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button(AppStrings.Common.create) {
                        Task {
                            if let (sldId, siteName) = await viewModel.createSite(modelContext: modelContext) {
                                dismiss()
                                if let onSiteCreated = onSiteCreated {
                                    onSiteCreated(sldId, siteName)
                                } else {
            ...
            .sheet(isPresented: $viewModel.showAccountPicker) {
                SearchableItemPickerView(
                    ...
                    selectedId: viewModel.selectedAccount.map { $0.id.uuidString },
                    idToString: { $0.id.uuidString },
                    nameOf: { $0.name },
            ...
            .sheet(isPresented: $viewModel.showLaborUnionPicker) {
                SearchableItemPickerView(
                    ...
                    selectedId: viewModel.selectedLaborUnion.map { $0.id },
                    idToString: { $0.id },
                    nameOf: { $0.name },
            ...
            .sheet(isPresented: $viewModel.showOfficePicker) {
                SearchableItemPickerView(
                    ...
                    selectedId: viewModel.selectedOffice.map { $0.id.lowercased() },
    ...
    private var officePickerRow: some View {
        Button { viewModel.showOfficePicker = true } label: {
            HStack {
                ...
                .buttonStyle(.plain)
                .infoTooltip(
                    isPresented: $showOfficeInfo,
                    title: AppStrings.Site.office,
                    message: AppStrings.Site.officeInfo
                )
                Spacer()
        ...
        Button { viewModel.showAccountPicker = true } label: {
            HStack {
                ...
                .infoTooltip(
                    ...
                    title: AppStrings.Site.account,
                    message: AppStrings.Site.accountInfo
                )
                Spacer()
    ...
    private var complexityPickerRow: some View {
        Button { viewModel.showComplexityPicker = true } label: {
            HStack {
                ...
                .buttonStyle(.plain)
                .infoTooltip(
                    isPresented: $showComplexityInfo,
                    title: AppStrings.Site.complexityLevel,
                    message: AppStrings.Site.complexityTooltip
                )
                Spacer()
    ...
    private var laborUnionPickerRow: some View {
        Button { viewModel.showLaborUnionPicker = true } label: {
            HStack {
                ...
                .buttonStyle(.plain)
                .infoTooltip(
                    isPresented: $showLaborUnionInfo,
                    title: AppStrings.Site.laborUnion,
                    message: AppStrings.Site.laborUnionInfo
                )
                Spacer()
        ...
        if viewModel.isCreating {
            ZStack {
                ...
                VStack(spacing: 20) {
                    ProgressView()
                        .scaleEffect(1.5)
                    Text(AppStrings.Site.creatingSite)
                        .font(.headline)
                }
                .padding(30)
...
/// Formats an Office for display in the picker — appends `(LANG)` when set.
func officeDisplayLabel(_ office: OfficeDTO) -> String {
    if let lang = office.language?.trimmed.nilIfEmpty {
        return "\(office.name) (\(lang.uppercased()))"
    }
    return office.name
}

private extension View {
    /// Tooltip-style info popover used by the picker rows. `presentationCompactAdaptation(.popover)`
    /// keeps it as a popover (with arrow) on iPhone instead of falling back to a sheet.
    func infoTooltip(isPresented: Binding<Bool>, title: String, message: String) -> some View {
