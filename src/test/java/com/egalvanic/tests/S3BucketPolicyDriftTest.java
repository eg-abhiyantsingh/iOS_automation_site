...
public class S3BucketPolicyDriftTest {
    ...
    // Precondition flags — set in @BeforeClass, checked by each bucket test.
    // This replaces dependsOnMethods so individual tests can run from the IDE.
    private static boolean awsAccessVerified = false;
    private static boolean baselinesVerified = false;
    private static String preconditionError = null;
    ...
    public void classSetup() {
        ...
        // Run precondition checks once — results cached in static flags.
        // Individual bucket tests check these flags instead of using dependsOnMethods,
        // which allows running any single test from the IDE without TestNG errors.
        awsAccessVerified = S3PolicyChecker.verifyAwsAccess();
        if (!awsAccessVerified) {
            preconditionError = "AWS access verification failed. Install AWS CLI and configure profile '"
                + S3PolicyChecker.getAwsProfile() + "'.";
            System.out.println("  [SMOKE] PRECONDITION FAILED: " + preconditionError);
            return;
        }
        System.out.println("  [SMOKE] AWS access verified");

        baselinesVerified = S3PolicyChecker.verifyBaselinesExist();
        if (!baselinesVerified) {
            preconditionError = "Baseline policy files missing or incomplete. Expected 40 files in: "
                + S3PolicyChecker.getBaselinesDir();
            System.out.println("  [SMOKE] PRECONDITION FAILED: " + preconditionError);
            return;
        }
        System.out.println("  [SMOKE] Baseline files verified");
    }

    /**
     * Checks that AWS access and baseline files are available.
     * Called at the start of each bucket test — skips the test if preconditions failed.
     */
    private void ensurePreconditions() {
        if (!awsAccessVerified || !baselinesVerified) {
            throw new SkipException("SKIPPED — precondition failed: " + preconditionError);
        }
    }
