...
public class LocationTest extends BaseTest {
    ...
    /**
     * TC_EB_004: Verify Cancel discards edit changes
     * 
     * Precondition: Edit Building screen with modified data
     * 
     * Steps:
     * 1. Modify Building Name
     * 2. Click Cancel
     * 3. Verify original data retained in list
     * 
     * Expected Result: Changes discarded, original building data unchanged
     */
    @Test(priority = 21)
    public void TC_EB_004_verifyCancelDiscardsEditChanges() {
        ExtentReportManager.createTest(
            AppConstants.MODULE_BUILDING,
            AppConstants.FEATURE_EDIT_BUILDING,
            "TC_EB_004 - Verify Cancel discards edit changes"
        ...
        String originalName = buildingToEdit;
        logStep("Original building name: " + originalName);
        ...
        // Get the actual pre-filled name from edit screen
        String prefilledName = buildingPage.getEditBuildingNameValue();
        logStep("Pre-filled name in edit screen: " + prefilledName);

        // Step 1: Modify Building Name (make a significant change)
        String modifiedName = "CANCELLED_CHANGE_" + System.currentTimeMillis();
        logStep("Step 1: Modifying Building Name to: " + modifiedName);
        buildingPage.clearBuildingName();
        shortWait();
        buildingPage.enterBuildingName(modifiedName);
        shortWait();

        // Verify the modification was made
        String currentName = buildingPage.getBuildingNameValue();
        assertTrue(currentName.contains("CANCELLED_CHANGE"), 
            "Building Name should show modified value before cancel");
        logStepWithScreenshot("Building Name modified - ready to test Cancel");
        ...
        logStep("Step 3: Verifying original data is retained (changes were discarded)");
        
        // The modified name should NOT appear in the list
        boolean modifiedNameExists = buildingPage.isBuildingDisplayed(modifiedName) ||
                                     buildingPage.isBuildingDisplayed("CANCELLED_CHANGE");
        assertFalse(modifiedNameExists, 
            "Modified name should NOT appear in list - changes should be discarded");

        // The original building should still exist
        // Note: We search for the original name or similar
        boolean originalExists = buildingPage.isBuildingDisplayed(prefilledName) ||
                                buildingPage.isBuildingDisplayed(originalName) ||
                                buildingPage.areBuildingEntriesDisplayed();
        assertTrue(originalExists, "Original building should still exist with original name");

        logStep("✓ Cancel successfully discarded changes - original data retained");
        logStepWithScreenshot("TC_EB_004: Cancel discards changes verification complete");
    }
